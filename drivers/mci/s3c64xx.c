...
#define to_s3c_pdata(pdata) container_of(pdata, struct s3c_sdhc_platform_data, esdhc_pd)
...
void esdhcp_interface_setup(struct esdhc_mci *h, unsigned freq)
{
	...
	if (freq > 25000000) {
		...
		esdhc_tx_delay_set(h, pd->tx_delay25);
		esdhc_rx_delay_set(h, pd->rx_delay25);
	} else {
		...
		if (pd->tx_delay_en)
			reg |= CONTROL2_ENFBCLKTX;
		esdhc_tx_delay_set(h, pd->tx_delay);
		esdhc_rx_delay_set(h, pd->rx_delay);
	}
...
unsigned esdhcp_get_base_clk(struct esdhc_mci *h)
{
	struct device_d *dev = h->mci.hw_dev;
	struct s3c_sdhc_platform_data *pd = to_s3c_pdata(h->pdata);
...
static int esdhc_s3c64xx_probe(struct device_d *dev)
{
	...
	this->mci.f_min = pd->esdhc_pd.f_min == 0 ? esdhcp_get_base_clk(this) / 256 : pd->esdhc_pd.f_min;
	this->mci.f_max = pd->esdhc_pd.f_max == 0 ? esdhcp_get_base_clk(this) / 2 : pd->esdhc_pd.f_max;
